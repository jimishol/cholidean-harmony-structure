...
function M.assignAll(scene, matLib, noteSystem, categoryMap)
  ...
  for category, matKey in pairs(map) do
    ...
    for idx, obj in ipairs(items) do
      ...
      -- Override colors for bass/topVoice only on joints
      if category == "joints" then
        if note.isBass and constants.bassColor then
          r, g, b = table.unpack(constants.bassColor)
        elseif note.isTopVoice and constants.topVoiceColor then
          r, g, b = table.unpack(constants.topVoiceColor)
        end
      end
  ...
  -- Bass and topVoice notes can override label color if configured
  for i, lbl in ipairs(scene.labels_to_Draw) do
    ...
    if mesh and mesh._matInst then
      ...
      local r, g, b = lbl.color[1], lbl.color[2], lbl.color[3]

      -- Apply bass/topVoice overrides on labels
      local note = notes[i]
      if note then
        if note.isBass and constants.bassColor then
          r, g, b = table.unpack(constants.bassColor)
  ...
  -- No bass/topVoice overrides here
